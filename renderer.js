...
if (!store.get("settings.newTabWallpaper")) {
  store.set("settings.newTabWallpaper", {
    name: "Reborn5",
    username: "Opera Software",
    html: "https://addons.opera.com/tr/wallpapers/details/reborn5/",
    backgroundTheme: "assets://images/newtab/opera.jpg",
  });
...
if (!store.get("vpn-proxy-data")) {
  store.set("vpn-proxy-data", [
    {
      id: 1,
      name: "India Hazratpur",
      co: "India TEST",
      ip: "117.251.103.186",
      port: "8080",
      protocol: "HTTPS",
      connection: "117.251.103.186:8080",
      security: "no",
    },
    {
      id: 2,
      name: "Mozambique Maxixe",
      co: "Mozambique TEST",
      ip: "41.76.145.136",
      port: "8080",
      protocol: "HTTPS",
      connection: "41.76.145.136:8080",
      security: "no",
    },
...
if (store.get("settings.leftbarmenu")) {
  ...
  if (store.get("settings.generalViews") == "OPERA") {
    ...
    document.getElementsByClassName("ntabsedits")[0].style.marginLeft =
      store.get("settings.leftbarmenuSize") + "px";
    document.getElementById("bookmarksAlls").style.paddingLeft =
      store.get("settings.leftbarmenuSize") + "px";
  } else {
    document.getElementById("menusleftbar").style.display = "block";
    document.getElementsByClassName("ntabsedits")[0].style.paddingLeft =
      store.get("settings.leftbarmenuSize") + "px";
    document.getElementById("bookmarksAlls").style.paddingLeft =
      store.get("settings.leftbarmenuSize") + "px";
  }
...
if (store.get("--incognitotab") == "--incognitotab") {
  ...
  if (!document.querySelector('link[href="systemui/css/incognitotab.css"]'))
    document.head.innerHTML +=
      '<link rel="stylesheet" href="systemui/css/incognitotab.css">';
  setTimeout(() => {
...
function searchBounds() {
  ...
  bounds.y =
    Math.ceil(
      navCenter.getBoundingClientRect().top +
        window.scrollY +
        parseFloat(getComputedStyle(navCenter, null).height.replace("px", ""))
    ) + 5;
  bounds.width = Math.floor(
    parseFloat(getComputedStyle(navCenter, null).width.replace("px", ""))
  );
  bounds.height = 240;
...
web.init(document);
shortcuts.init(keyboardShortcut, (n) => {
  if (tabs.tabs[n - 1]) tabs.activate(tabs.tabs[n - 1]);
});

console.colorLog = (msg, color) => {
...
exports.getTabCountCacheNum = function () {
  store.set("tabslengthOnlys", tabs.tabs.length);
};
...
ipcMain.on("getBookmarks", async (e) => {
  e.returnValue = await storage.getBookmarks();
});
...
ipcMain.on("getHistory", async (e) => {
  e.returnValue = await storage.getHistory();
});
...
ipcMain.on("getSaveInfoXcont", async (e) => {
  e.returnValue = await storage.getSaveInfoXcont();
});
ipcMain.on("clearSaveInfoXcont", async () => storage.clearSaveInfoXcont());
ipcMain.on("removeSaveInfoXcontItem", async (e, id) =>
  storage.removeSaveInfoXcontItem(id)
);
ipcMain.on("logSaveInfoXcont", async (e, site, title, pass, loginurl) => {
  storage
    .isgetSaveInfoXconted(site, title, pass)
    .then((isgetSaveInfoXconted) => {
      if (isgetSaveInfoXconted) {
      } else {
        autoFills(site, title, pass, loginurl);
      }
    });
});
  ...
  storage
    ...
    .then((isgetSaveInfoXconted) => {
      if (isgetSaveInfoXconted) {
        ...
        storage.logSaveInfoXcont(site, title, pass, loginurl);
      }
    });
});
...
ipcMain.on("logtabsPins", async (e, site, id) => {
  if (newtabBased == "incognito-new-tab") {
    ...
    storage.isgettabsPins(id).then((isgettabsPins) => {
      if (isgettabsPins) {
        storage.removetabsPins(id);
      } else {
        storage.logtabsPins(site, id);
      }
    });
  }
});
ipcMain.on("gettabsPins", async (e) => {
  e.returnValue = await storage.gettabsPins();
});
...
ipcMain.on("getDownloads", async (e) => {
  e.returnValue = await storage.getDownloads();
});
ipcMain.on("clearDownloads", async () => storage.clearDownloads());
ipcMain.on("removeDownloads", async (e, id) => storage.removeDownloads(id));

ipcMain.on("getShellGo", async (e, url) =>
  shell.openPath(url.replace(/\\/g, "/"))
);
...
ipcMain.on("getbird", async (e) => {
  e.returnValue = await storage.getbird();
});
...
ipcMain.on("store", async (e, purpose, name, value) => {
  if (purpose == "set") {
    ...
    if (exmodules[0] == "extensions") {
      loadUzatilar();
      if (value) {
        document.getElementById(exmodules[1]).style.display = "flex";
      } else {
        document.getElementById(exmodules[1]).style.display = "none";
      }
    }
...
ipcMain.on("settingsViewStatusSet", async (e, action) => {
  ...
  if (action == "ss") {
    e.returnValue = store.get("settings.ss");
  }
...
ipcMain.on("siteInfo", async (e, action) => {
  switch (action) {
    case "Certificate":
      ...
      req.on("error", () => {
        showAlert({
          type: "alert",
          message: i18n.__(
            "Sitenin SSL sertifikası yok yada geçersiz bir SSL sertifikası kullanıyor."
          ),
          url: "BLOCK",
        });
      });
    ...
    case "site-info":
      ...
      req_1.on("error", () => {
        if (!tabs.current().webContents.getURL().includes("block://")) {
          toggleSiteInfo(false);
        }
      });
    ...
    case "realseUpdateInfo":
      ...
      tabs.newView(
        "https://github.com/cetinmert/block/releases/tag/v" +
          urlGetInfoRealse.version
      );
      break;
...
ipcMain.on(
  "getVersions",
  async (e) =>
    (e.returnValue = { ...process.versions, block: version, env: process.env })
);

ipcMain.on("getThemes", async (e) =>
  require("fs").readdir(
    join(__dirname, "systemui/css/themes"),
    (err, files) => {
      let result = [];
...
ipcMain.on(
  "viewSourceURL",
  async (e) => (
    (e.returnValue = store.get("viewSourceURL")), store.delete("viewSourceURL")
  )
);
ipcMain.on(
  "viewSourceMAINCode",
  async (e) => (
    (e.returnValue = store.get("viewSourceMAINCode")),
    store.delete("viewSourceMAINCode")
  )
);
...
ipcMain.on(
  "st_au_app_download",
  async (e) => (e.returnValue = store.get("stroe_au_app_download"))
);
ipcMain.on(
  "st_update_downloaded",
  async (e) => (e.returnValue = store.get("stroe_update_downloaded"))
);
ipcMain.on(
  "st_au_releaseInfo",
  async (e) => (e.returnValue = store.get("stroe_au_releaseInfo"))
);

//Ayarlar Tema, Üst Header Güncelleme
ipcMain.on(
  "loadThemeNews",
  async (e) => ((e.returnValue = "true"), loadTheme())
);
ipcMain.on(
  "loadHeaderViews",
  async (e) => ((e.returnValue = "true"), loadHeaderViews())
);

//Gerçerli Sekmedeki Url Çek
ipcMain.on(
  "getCurrentURLTabss",
  async (e) => (e.returnValue = tabs.current().webContents.getURL())
);
...
ipcMain.on(
  "vpnproxyip",
  async (e) => (e.returnValue = store.get("vpn-proxy-ip"))
);
...
ipcMain.on(
  "vpnproxyopendstatus",
  async (e) => (e.returnValue = store.get("vpn-proxy-opend"))
);

ipcMain.on("vpnproxyopend", async (e) => {
  var vpnproxyopend = store.get("vpn-proxy-opend");
  if (vpnproxyopend) {
    document.getElementById("vpnimagevpn").src = join(
      __dirname,
      "static/extensions/vpn/BLOCK-VPN-Closed.png"
    );
    ...
    document.getElementById("vpnimagevpn").src = join(
      __dirname,
      "static/extensions/vpn/BLOCK-VPN.png"
    );
...
async function keyboardShortcut(shortcut) {
  switch (shortcut) {
    ...
    case "devTools":
      if (
        !tabs.current().webContents.getURL().includes("block://") &&
        !isDevMode
      )
    ...
    case "openHistory":
      tabs.newView("block://history");
      break;
    case "openBookmarks":
      tabs.newView("block://bookmarks");
      break;
    case "openSettings":
      tabs.newView("block://settings");
      break;
    default:
      break;
  }
...
ipcMain.on(
  "openPageNews",
  async (e, a) => ((e.returnValue = "true"), tabs.newView(a))
);
...
ipcMain.on("newWinOpenSettings", async (e, a) =>
  tabs.newView("block://settings")
);
ipcMain.on("newWinOpenHistory", async (e, a) =>
  tabs.newView("block://history")
);
ipcMain.on("newWinOpenBookMarks", async (e, a) =>
  tabs.newView("block://bookmarks")
);
ipcMain.on("newWinOpenDowloand", async (e, a) =>
  tabs.newView("block://downloads")
);
ipcMain.on("newWinOpenHakkinda", async (e, a) =>
  tabs.newView("block://version")
);
...
async function autoFills(site, title, pass, loginurl) {
  var autofillss = new BrowserWindow({
    ...
    x:
      Math.ceil(
        document.getElementById("bookmark").getBoundingClientRect().left +
          window.screenX
      ) - 280,
    y: Math.ceil(
      document.getElementById("bookmark").getBoundingClientRect().top +
        window.screenY +
        parseFloat(
          getComputedStyle(
  ...
  var autodatafill = [
    { xsite: site, xtitle: title, xpass: pass, xloginurl: loginurl },
  ];
...
async function loadTheme() {
  if (newtabBased == "incognito-new-tab") {
    ...
    if (themeObj) {
      themeObj = themeObj.toLowerCase();
    }

    if (document.querySelector('head link[href*="systemui/css/themes"]'))
      document.querySelector('head link[href*="systemui/css/themes"]').remove();
    document.head.innerHTML +=
      '<link rel="stylesheet" href="systemui/css/themes/' + themeObj + '.css">';
    ...
    if (themeObj == "default") {
      if (thmname == "OPERA") {
        if (document.querySelector('head link[href*="systemui/css/themes"]'))
          document
            .querySelector('head link[href*="systemui/css/themes"]')
            .remove();
        document.head.innerHTML +=
          '<link rel="stylesheet" href="systemui/css/themes/' +
          themeObj +
          '-opera.css">';
...
async function loadHeaderViews() {
  ...
  if (HeaderObj) {
    HeaderObj = HeaderObj.toLowerCase();
  }
  ...
  let ckfgt = document.body.classList.contains("compact")
    ? "compact"
    : "default";
  if (HeaderObj != ckfgt) {
...
async function fullscrenndoms() {
  ...
  let address = require("url").format({
    pathname: join(
      __dirname,
      "layout/pages/dialogs/modal/fullscreenchange.html"
    ),
    protocol: "file:",
    slashes: true,
  });
...
async function loadPage(val) {
  if (
    tabs.current().webContents.getURL().startsWith("block:") ||
    tabs.current().webContents.getURL().startsWith("file:")
  ) {
    loadpagev2s(val);
  } else {
    ...
    try {
      ...
      if (val == "block://new-tab") {
        if (store.get("settings.newtabmode") == "block") {
          tabs.current().webContents.loadURL(
            require("url").format({
              pathname: join(
                __dirname,
                "layout/pages/" + newtabBased + oppayancar + ".html"
              ),
              protocol: "file:",
              slashes: true,
            })
...
async function toggleSiteInfo(certificate) {
  if (!siteInfo) {
    siteInfo = new BrowserWindow({
      ...
      height: 330,
      x: Math.ceil(
        document.getElementById("site-info").getBoundingClientRect().left +
          window.screenX
      ),
      y: Math.ceil(
        document.getElementById("site-info").getBoundingClientRect().top +
          window.screenY +
          parseFloat(
            getComputedStyle(
...
document
  .getElementById("back")
  .addEventListener("click", async () => keyboardShortcut("backPage"));
document
  .getElementById("forward")
  .addEventListener("click", async () => keyboardShortcut("forwardPage"));
...
ipcMain.on("createleftmenuopen", async (e, item) => {
  document.getElementById(item.name).click(); /*createLeftMenuAres(item)*/
});

ipcMain.on(
  "createLeftMenuAresOpenClosed",
  async (e, name, type, xwidht, xurl) => {
...
async function createLeftMenuAresOpenClosed(name, type, xwidht, xurl) {
  ...
  if (cache_viewsx) {
    ...
    if (type == "close") {
      ...
      windowxviewW = -store.get("settings.leftbarmenuSize");
      if (store.get("settings.generalViews") == "OPERA") {
        document.getElementById("newopclss").style.marginLeft =
          store.get("settings.leftbarmenuSize") - 10 + "px";
        document.getElementsByClassName("ntabsedits")[0].style.marginLeft =
          store.get("settings.leftbarmenuSize") + "px";
        document.getElementById("bookmarksAlls").style.paddingLeft =
          store.get("settings.leftbarmenuSize") + "px";
      } else {
        document.getElementById("plugingoreas").style.paddingLeft =
          store.get("settings.leftbarmenuSize") + "px";
        document.getElementsByClassName("ntabsedits")[0].style.paddingLeft =
          store.get("settings.leftbarmenuSize") + "px";
        document.getElementById("bookmarksAlls").style.paddingLeft =
  ...
  if (!cache_viewsx) {
    ...
    if (url.startsWith("block:")) {
      ...
      viewsx = new BrowserWindow({
        ...
        x:
          Math.ceil(
            document.getElementById("menusleftbar").getBoundingClientRect()
              .left + window.screenX
          ) + xleftbarmenuSize,
...
if(disposition == 'new-window'){
    ...
    if (url.startsWith("block:")) {
      ...
      var autogetdates = [
        { xname: name, xurl: url, xwidht: xwidht, xkeyid: keyid },
      ];
    ...
    viewsx.webContents.session.webRequest.onBeforeSendHeaders(
      (details, callback) => {
        if (details.url) {
        ...
        if (store.get("flags").includes("--do-not-track"))
          details.requestHeaders["DNT"] = "1"; // Enable DNT for 'do-not-track' flag
        ...
        if (cache_viewsx) {
          ...
          if (cache_viewsx.webContents.getURL().startsWith("file:")) {
            cache_viewsx.setBounds({
              width: window.outerWidth + windowxviewW - leftopagam,
              height:
                window.outerHeight - topbarHeight - leftopagam - leftopagam,
              x:
                Math.ceil(
                  document
                    .getElementById("menusleftbar")
                    .getBoundingClientRect().left + window.screenX
                ) + xleftbarmenuSize,
              y: window.screenY + topbarHeight + leftopagam,
            });
          } else {
...
async function createWindowMenuTs() {
  let mainWindowMenus = new BrowserWindow({
    ...
    y:
      Math.ceil(
        document.getElementById("menu").getBoundingClientRect().top +
          window.screenY +
          parseFloat(
            getComputedStyle(
...
document.getElementById("url").addEventListener("keypress", async (e) => {
  if (e.which == 13 || e.which == 10) {
    if (e.ctrlKey) {
      document.getElementById("url").value =
        "www." + document.getElementById("url").value;
      document.getElementById("url").value =
        document.getElementById("url").value + ".org";
    } else if (e.shiftKey) {
      document.getElementById("url").value =
        "www." + document.getElementById("url").value;
      document.getElementById("url").value =
        document.getElementById("url").value + ".net";
    } else {
...
document.getElementById("url").addEventListener("focus", async (e) => {
  e.preventDefault();
  document.getElementById("nav-center").style.border =
    "var(--accent) 2px solid";

  if (tabs.current()) {
    if (
      tabs.current().webContents.getURL().includes("block://") ||
      tabs.current().webContents.getURL().includes("file://")
    ) {
    } else {
      if (tabs.current().webContents.getURL()) {
        let dmans = new URL(tabs.current().webContents.getURL());
        if (dmans.hostname == "bing.com" || dmans.hostname == "www.bing.com") {
        } else {
          document.getElementById("url").value = tabs
            .current()
            .webContents.getURL();
        }
      }
...
document.getElementById("url").addEventListener("blur", async () => {
  ...
  document.getElementById("url").placeholder = document
    .getElementById("url")
...
async function createWindowUpdate() {
  let mainWindowUpdate = new BrowserWindow({
    ...
    x:
      Math.ceil(
        document.getElementById("system_update").getBoundingClientRect().left +
          window.screenX
      ) - 285,
    y: Math.ceil(
      document.getElementById("system_update").getBoundingClientRect().top +
        window.screenY +
        parseFloat(
          getComputedStyle(
            document.getElementById("system_update"),
            null
          ).height.replace("px", "")
        )
    ),
...
getSearchEngine(async (e) => {
  ...
  document
    .getElementById("url")
    .setAttribute("data-placeholder", serachmotorname);
});

async function setSearchEngine() {
  var engineMotors = store.get("settings.search_engine");
  document.getElementById("url").placeholder =
    i18n.__("Ara") + ` ${engineMotors} ` + i18n.__("yada url dene");
  document
    .getElementById("url")
    .setAttribute(
      "data-placeholder",
...
if (newtabBased == "incognito-new-tab") {
  tabs.newView(
    remote.process.argv[2] &&
      (remote.process.argv[2].startsWith("http") ||
        remote.process.argv[2].startsWith("block"))
      ? remote.process.argv[2]
      : "block://" + newtabBased
  );
  //document.getElementById('NewTab').click();
  ...
  fs.readdir(uzantilarKlasor, (err, files) => {
    files.forEach((file) => {
      ...
      var manifest = JSON.parse(
        fs
          .readFileSync(uzantilarKlasor + "/" + file + "/manifest.json")
          .toString()
      );
    ...
    if (store.get("settings.starter") == 2) {
      ...
      for (item of tabs.tabs) {
        ...
        if (!anatabssxx.classList.contains("tab-pins")) {
          let ggezmode_babe = document.getElementById(anatabssxx.id)
            .parentElement.id;

          var itemNewsUrlCac = {
            url: item.webContents.getURL(),
            ggezmode_babe: ggezmode_babe,
          };
...
async function windowPageAllResize() {
  ...
  for (ntabs of tabs.tabs) {
    ntabs.setBounds({
      x: windowxview,
      y: topbarHeight,
      width: win.getContentBounds().width + windowxviewW + opraidus,
      height: win.getContentBounds().height - topbarHeight + opraidus,
    });
    win.on("resize", () => {
      ntabs.setBounds({
        x: windowxview,
...
async function genelKitaplikDataPross() {
  if (store.get("settings.bookmarkViews")) {
    ...
    if (anaIcerikDataCleans) {
      anaIcerikDataCleans.innerHTML = "";
    }
