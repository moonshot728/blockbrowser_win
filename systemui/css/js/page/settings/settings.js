document.getElementById("version").innerText = sendSync("getVersions").block;
document.getElementById("dowloandsHEdef").innerText =
  sendSync("dowloandsHEdef");
...
const searchEngines = sendSync("store", "get", "searchEngines").map(
  (e) => e.name
);
...
function loadDataEngines(oneID, twoID, theFuc) {
  ...
  let options = searchEngines
    .map(
      (op) =>
        `<div class="sc-1k8lovv hJBLHH" onclick="${theFuc}('${op}', '${op}', '${twoID}')">${op}</div>`
    )
    .join("\n");
...
function openPopupModals(idElementx, closeEvents) {
  ...
  if (elementIdBul) {
    ...
    if (!closeEvents) {
      ...
      if (displayElements.display == "block") {
        displayElements.display = "none";
        elementIdCloseEv.style.display = "none";
      }
    }
...
function closeEventBodyAll() {
  openPopupModals("yenisekme-modu", true);
  openPopupModals("genel-tema-tasarimi", true);
...
/* Genel Tema Değiştirme İşlemi */
function genelthemechanger(eValue, eText, idElementxText) {
  var elementIdTextBul = document.getElementById(idElementxText);
  if (elementIdTextBul) {
    elementIdTextBul.innerHTML = eText;
    send("store", "set", "settings." + "generalViews", eValue);
    //sendSync('loadgeneralViews');
    //window.location.reload();
  }
}

/* Yeni Sekme Modu Değiştirme İşlemi */
function newtabmodechanger(eValue, eText, idElementxText) {
  var elementIdTextBul = document.getElementById(idElementxText);
  if (elementIdTextBul) {
    var namevalueNew = "";
    if (eText == "block") {
      namevalueNew = i18n.__("Varsayılan Yeni Sekme");
    }
    if (eText == "bing") {
      namevalueNew = i18n.__("BING Yeni Sekme");
    }

    elementIdTextBul.innerHTML = namevalueNew;
    send("store", "set", "settings." + "newtabmode", eValue);
    //sendSync('loadnewtabmode');
    //window.location.reload();
  }
}
...
function prossTemaRengi(eValue, eText, idElementxText) {
  var elementIdTextBul = document.getElementById(idElementxText);
  if (elementIdTextBul) {
    elementIdTextBul.innerHTML = i18n.__(eText);
    ...
    if (control.tagName == "DIV") {
      ...
      if (key == "theme") {
        ...
        if (intextkeys) {
          var namevalueNew = "";
          if (val == "default") {
            namevalueNew = i18n.__("Varsayılan");
          }
          if (val == "grey") {
            namevalueNew = i18n.__("Gri");
          }
          if (val == "midnightblue") {
            namevalueNew = i18n.__("Gece Mavisi");
          }
          if (val == "black") {
            namevalueNew = i18n.__("Siyah");
          }
          if (val == "beige-white") {
            namevalueNew = i18n.__("Bej - Beyaz");
          }

          if (val == "yellow-white") {
            namevalueNew = i18n.__("Sarı - Beyaz");
          }
          if (val == "green-white") {
            namevalueNew = i18n.__("Yeşil - Beyaz");
          }
          if (val == "light-turquoise-white") {
            namevalueNew = i18n.__("Açık Turkuaz - Beyaz");
          }
          if (val == "light-purple-white") {
            namevalueNew = i18n.__("Açık Mor - Beyaz");
          }
          if (val == "pink-white") {
            namevalueNew = i18n.__("Pembe - Beyaz");
          }

          if (val == "beige") {
            namevalueNew = i18n.__("Bej");
          }
          if (val == "orange") {
            namevalueNew = i18n.__("Turuncu");
          }
          if (val == "light-green") {
            namevalueNew = i18n.__("Açık Yeşil");
          }
          if (val == "light-turquoise") {
            namevalueNew = i18n.__("Açık Turkuaz");
          }
          if (val == "light-blue") {
            namevalueNew = i18n.__("Açık Mavi");
          }
          if (val == "pink") {
            namevalueNew = i18n.__("Pembe");
          }

          if (val == "dark-pink") {
            namevalueNew = i18n.__("Koyu Pembe");
          }
          if (val == "dark-red") {
            namevalueNew = i18n.__("Koyu Kırmızı");
          }
          if (val == "dark-green") {
            namevalueNew = i18n.__("Koyu Yeşil");
          }
          if (val == "dark-turquoise") {
            namevalueNew = i18n.__("Koyu Turkuaz");
          }
          if (val == "dark-blue") {
            namevalueNew = i18n.__("Koyu Mavi");
          }
          if (val == "dark-purple") {
            namevalueNew = i18n.__("Koyu Mor");
          }
      ...
      /*Genel Tema Ayarları Özel*/
      if (key == "generalViews") {
        var intextkeys = document.getElementById(key + "-text");
        if (intextkeys) {
          var namevalueNew = "";
          if (val == "BLOCK") {
            namevalueNew = i18n.__("BLOCK");
          }
          if (val == "OPERA") {
            namevalueNew = i18n.__("OPERA");
          }
          intextkeys.innerHTML = namevalueNew;
        }
      }
      /*Genel Tema header Ayarları Özel Son*/

      /*Yeni Sekme Modu Ayarları Özel*/
      if (key == "newtabmode") {
        var intextkeys = document.getElementById(key + "-text");
        if (intextkeys) {
          var namevalueNew = "";
      ...
      /*Yeni Sekme Modu Ayarları Özel Son*/

      /*Üst çubuk header Ayarları Özel*/
      if (key == "headerView") {
        var intextkeys = document.getElementById(key + "-text");
        if (intextkeys) {
          var namevalueNew = "";
          if (val == "default") {
            namevalueNew = i18n.__("Tam");
          }
          if (val == "compact") {
            namevalueNew = i18n.__("Kompakt");
          }
          intextkeys.innerHTML = namevalueNew;
        }
      }
      ...
      if (key == "langs") {
        ...
        if (intextkeys) {
          ...
          if (val == "tr") {
            namevalueNew = "Turkish";
          }
          if (val == "en") {
            namevalueNew = "English";
          }
          if (val == "ru") {
            namevalueNew = "Russian";
          }
          if (val == "de") {
            namevalueNew = "German";
          }
          if (val == "es") {
            namevalueNew = "Spanish";
          }
          if (val == "fr") {
            namevalueNew = "French";
          }
...
async function bindControls2(controls2) {
  for (let control2 of controls2) {
    ...
    control2.addEventListener("click", () =>
      send("store", "set", "settings." + key2, control2.value)
    );
  }
}
  ...
  for (let control of controls) {
    ...
    if (control.tagName == "SELECT") {
      control.addEventListener("change", () =>
        send("store", "set", "settings." + key, control.value)
      );
    } else {
      control.addEventListener("click", () =>
        send("store", "set", "settings." + key, control.checked)
      );
    }
  }
}
...
function loadDataLangs(oneID, twoID) {
  let panelEl = document.getElementById(oneID);
  let options = getLangPacs
    .map(
      (op) =>
        `

<div class="sc-1k8lovv hJBLHH" onclick="prossLangChanges('${op}', '${op}', 'langs-text')">${clangto(
          op
        )}</div>
`
    )
...
document.getElementById("restclear").addEventListener("click", async (e) => {
  var result = confirm(
    i18n.__(
      "Bu uygulamaya ait herşey silinecektir. Tarama Verileri, Ayarlar, Geçmiş, İndirilenler. Herşey Fabrika Ayarına Dönecektir."
    )
  );
  if (result) {
    send("factoryReset");
...
function setVersions(versions) {
  ...
  document.getElementById("block").innerHTML = versions.block;
}
...
if (leftMenuAffters) {
  leftMenuAffters.classList.add("bEUiyY");
}
...
getJSON(
  "https://api.github.com/repos/cetinmert/block/releases/latest",
  function (err, data) {
    if (err !== null) {
      ...
      if (!data.draft) {
        ...
        document.getElementById("iblockGen").innerText =
          sendSync("getVersions").block;

        document.getElementById("iblockYayiym").innerText = data.name;
        if (data.body.trim()) {
          document.getElementById("dahamasosss").style.display = "block";
        }
        ...
        document.getElementById(
          "githuburlss"
        ).innerHTML = `<a onclick="openLoadURLEx('${data.html_url}')">${data.html_url}</a>`;

        if (data.name <= sendSync("getVersions").block) {
          document.getElementById("sfotGuncelss").style.display = "block";
          document.getElementById("sfotGuncelssDegil").style.display = "none";
          document.getElementById(
            "sfotGuncelss"
          ).innerHTML = `<div onclick="openLoadURLEx('${
            data.html_url
          }')" class="sc-v03tst epMHXf" id="sfotGuncelssx">${i18n.__(
            "Sistem Güncel, Son Sürüm Kullanıyor"
          )}</div>`;
        } else {
          ...
          document.getElementById(
            ...
            data.html_url
          }')" class="sc-v03tst epMHXf" id="sfotGuncelssDegilx">${i18n.__(
            "Sistem Güncel Değil, Yazılımı Güncelle!"
          )}</div>`;
        }
      } else {
        document.getElementById("defaultSoftWare").style.display = "block";
