...
if (store.get("settings.generalViews") == "OPERA") {
  ...
  win.on("resized", function () {
    if (cachetabview) {
      cachetabview.setBounds({
        x: windowxview,
        y: topbarHeight,
        width: win.getContentBounds().width + windowxviewW + opraidus,
        height: win.getContentBounds().height - topbarHeight + opraidusalt,
      });
    }
  });
...
function realNowtopbarHeight() {
  ...
  if (store.get("settings.topbarHeight")) {
    topBarOlcus = store.get("settings.topbarHeight");
  }
  return topBarOlcus;
}
...
*/
    ...
    if (!this.downloadWindowShows) {
      if (store.get("settings.dowloadnAltPencere")) {
        downloadWindow.show();
      }
    ...
    item.once("done", async (event, state) => {
      if (state === "completed") {
        ...
        if (extname(item.getFilename()) === ".crx") {
          if (store.get("settings.dowloadnAltPencere")) {
            downloadWindow.webContents.send("InstallCRX", id, savePath);
          }
          ...
          if (await fsss.existsSync(path)) {
            ...
            if (store.get("settings.dowloadnAltPencere")) {
              downloadWindow.webContents.send("NotInstallCRX", id, savePath);
            }
            return;
          }
          ...
          if (store.get("settings.dowloadnAltPencere")) {
            downloadWindow.webContents.send("CompleteInstallCRX", id, savePath);
          }
          setTimeout(() => {
...
exports.initBrowserView = async (view) => {
  ...
  view.webContents.on("did-fail-load", async (e, ec, ed, vu) => {
    web.failLoad(e, view, ec, ed, vu);
  });
...
if(disposition == 'new-window'){
  ...
  view.webContents.on("media-paused", async (e) => {
    setTimeout(() => {
      if (view.webContents.getURL()) {
        ...
        if (
          !view.webContents.getURL().includes("block://") &&
          !view.webContents.getURL().includes("file://")
        ) {
  ...
  view.webContents.on("page-title-updated", async (e, t) => {
    web.titleUpdated(view, e, t);
  });
  view.webContents.on("did-navigate", async (e, url) => {
    web.didNavigate(url, view, storage);
  });
  view.webContents.on("did-navigate-in-page", async (e, url) => {
    web.didNavigate(url, view, storage);
  });
  view.webContents.on("preload-error", async (e, path, err) => {
    console.error("PRELOAD ERROR", err);
  });

  //bekle
  view.webContents.on("login", async (e, path, err) => {
    console.log("ok");
  });
...
exports.savePage = function (contents) {
  let filters = [
    ...
    {
      name: i18n.__("Web Sayfası, Yalnızca HTML"),
      extensions: ["html", "htm"],
    },
  ...
  dialog.showSaveDialog(options).then((det) => {
    if (!det.cancelled) {
      ...
      contents
        .savePage(path, saveType)
        .then(() => {
          let input = {
            message: i18n.__("Sayfa başarıyla kaydedildi."),
            type: "alert",
            url: "BLOCK",
          };
...
exports.newView = async function (url, active = true, pinsid, ggezmode_babe) {
  ...
  /* VPN - Proxy */
  var proxyList;
  if (store.get("vpn-proxy-opend")) {
    proxyList = store.get("vpn-proxy-ip");
  } else {
    proxyList = "";
  }
...
Whatsapp this problem has been reported and help is awaited.*/
  ...
  tabSession.cookies.on("changed", async (e, cookie, cause, rem) => {
    if (!rem) {
      ...
      try {
        ...
        if (domain != host) {
          if (host == "google.com" || host == "youtube.com") {
          } else {
  ...
  // CUSTOM PROTOCOLS
  tabSession.protocol.registerHttpProtocol(
    "ipfs",
    (req, cb) => {
  ...
  tabSession.webRequest.onResponseStarted(async (det) => {
    let type =
      det.responseHeaders["Content-Type"] ||
      det.responseHeaders["content-type"];
    ...
    if (resource == "mainFrame" && type[0].includes("application/json")) {
      if (store.get("settings.json_viewer")) {
        view.webContents.loadURL(
          join(__dirname, "..", "module", "json-viewer", "index.html") + query
        );
      }
  ...
  tabSession.protocol.registerFileProtocol(
    "assets",
    (req, cb) => {
...
/*/
        ...
        if (url.pathname == "//incognito-new-tab") {
          if (newtabBased == "incognito-new-tab") {
            if (url.pathname == "//new-tab") {
              cb(
                join(__dirname, "../../layout/pages/", `incognito-new-tab.html`)
              );
            } else {
  ...
  tabEl.innerHTML =
    `<img id="${v1ids}" class="tab-icon" src="systemui/images/loading-${tone}.gif">
...
<img id="${v1ids}" class="tab-close" src="systemui/images/close.svg">`.trim();
  ...
  if (ggezmode_babe) {
    if (ggezmode_babe == "tabs") {
      document
        ...
        .insertAdjacentElement("beforebegin", tabEl);
    } else {
    ...
    if (document.getElementsByClassName("tab selected")[0]?.id) {
      document
        .getElementById(document.getElementsByClassName("tab selected")[0].id)
        .insertAdjacentElement("afterend", tabEl);
    } else {
      document
        .getElementById("new-tab")
        .insertAdjacentElement("beforebegin", tabEl);
  ...
  async function tabmenuss(view, xid) {
    ...
    let tabMenuTemp = [
      ...
      {
        label: i18n.__("Yenile"),
        click: async () => {
          ipcRenderer.send("openPage", view.webContents.getURL());
        },
      },

      {
        label: view.webContents.isAudioMuted()
          ? i18n.__("Sitenin Sesini Aç")
          : i18n.__("Sitenin Sesini Kapat"),
        click: async () => {
          if (!view.webContents.isAudioMuted()) {
      ...
      {
        label: document.getElementById(xid).classList.contains("tab-pins")
          ? i18n.__("Sabitlemeyi Kaldır")
          : i18n.__("Sabitle"),
        click: async () => {
    ...
    if (store.get("settings.newtabmode") == "block") {
      view.webContents.loadURL(
        require("url").format({
          pathname: join(
            __dirname,
            "../../layout/pages/new-tab" + oppayancar + ".html"
          ),
          protocol: "file:",
          slashes: true,
        })
...
exports.viewActivated = function (view) {
  web.changeTab(view, storage);
};
exports.viewAdded = function (view) {
  this.tabs.push(view);
  ipcRenderer.send("viewAdded");
};
...
document
  ...
  .addEventListener("click", async () =>
    this.newView("block://" + newtabBased + oppayancar)
  );
/*
...
ipcMain.on(
  "show-context-menu",
  async (e, rightClickPosition, selectionText, mediaType, srcURL, linkURL) => {
    ...
    const template = [
      ...
      },
      {
        ...
        //visible: selectionText.length == 0,
        click: async () => {
          this.savePage(view.webContents);
        },
      },
      {
        type: "separator",
      ...
      },
      {
        ...
        visible: mediaType === "img",
        click: async () => {
          view.webContents.downloadURL(srcURL);
        },
      },
      {
        ...
        click: async () => {
          this.newView(linkURL);
        },
      },
      {
        label:
          i18n.__("Google İle Ara") + " “" + abbrev_name(selectionText) + "”",
...
*/
      ...
      },
      {
        ...
        click: async () => {
          ...
          view.webContents.inspectElement(
