...
},
{
  ...
  {
    name: "arkhamintelligence",
    url: "https://platform.arkhamintelligence.com/signup?referrer=YjJmYzQ4OWEwNTQyM2JlMmYwZjc0MWFiYjI5ODI5NWUwZjYyODM4YjBjNDdjYmYwNzlkZDQ1NTg0ODZmYWMwODRkMGM1Yzg0YzczYjYzZWM4NmUxN2FlZGI0MTAyMDE3Mzk",
    status: true,
    widht: 500,
    uisytem: false,
  },
  {
    name: "zerion",
    url: "https://app.zerion.io/tokens/ETH-eth",
    status: true,
    widht: 500,
    uisytem: false,
  },
  ...
  {
    name: "Apple Music",
    url: "https://music.apple.com/tr/browse",
    status: true,
    widht: 500,
    uisytem: false,
  },
...
if (!store.get("settings.generalViews")) {
  store.set("settings.generalViews", "OPERA");
}
//TYPE = 58 OR 40
...
process.env.GOOGLE_DEFAULT_CLIENT_ID =
  "590185167148-f09884kcvf5p736tcv9ahjtepttbttk6.apps.googleusercontent.com";
...
!fsss.existsSync(extensionsPath) &&
  fsss.mkdirSync(extensionsPath, { recursive: true });
      ...
      if (details.url.startsWith("chrome-extension")) {
        ...
        if (idexp) {
          mainWindow.webContents.executeJavaScript(
            `document.getElementsByTagName("browser-action-list")[0].shadowRoot.getElementById("${idexp}").click();`
          );
        } else {
        ...
        mainWindow.webContents.executeJavaScript(
          'document.getElementsByTagName("browser-action-list")[0].shadowRoot.getElementById("nkbihfbeogaeaoehlefnkodbefgpgknn").click();'
        );
      }
    },
  });
...
async function createUpdater() {
  ...
  autoUpdater.on("update-available", (info) => {
    if (info) {
      store.set("stroe_au_releaseInfo", info);
    }
...
app.on("ready", () => {
  ...
  if (!store.get("appGetPath")) {
    store.set("appGetPath", app.getPath("downloads"));
  }
  //app.on('login');
});
